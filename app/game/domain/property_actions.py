...
class PropertyActionContext:
    ...
    append_game_over_if_last_survivor: Callable[
        [GameState, list[dict], list[dict]], None
    ]
...
def apply_property_acquisition(
    ...
    *,
    ...
    if (
        board_tile is None
        or tile_def is None
        or board_tile.tile_type != TileType.PROPERTY
    ):
    ...
    patches = [
        ...
        op_set(
            f"players.{player_id}.building_levels.{tile_id}", tile_def.building_level
        ),
    ]
...
def apply_purchase(
    ...
    *,
    ...
    if tile_state.owner_id is not None:
        raise GameActionError(
            code="INVALID_PHASE", message="이미 소유자가 있는 타일입니다."
        )
...
def apply_toll_payment(
    ...
    *,
    ...
    if (
        ...
        raise GameActionError(
            code="INVALID_TILE", message="통행료를 지불할 수 없는 타일입니다."
        )

    owner_id = tile_state.owner_id
    if owner_id is None or owner_id == player_id:
        raise GameActionError(
            code="INVALID_PHASE", message="통행료를 지불할 대상이 없습니다."
        )
